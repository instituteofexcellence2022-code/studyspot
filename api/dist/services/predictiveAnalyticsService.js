...
class PredictiveAnalyticsService {
  ...
  predictOccupancy(library, datetime, historicalData = []) {
    ...
    const seasonFactor = this.isWeekend(date) ? this.seasonalFactors.weekend : this.seasonalFactors.semester;

    // Library popularity factor
    const popularityFactor = (library.rating || 4.0) / 5.0;

    // Calculate predicted occupancy
    const predictedOccupancy = Math.round(baseOccupancy * dayFactor * hourFactor * seasonFactor * popularityFactor);
  ...
  suggestPricing(library, datetime) {
    const prediction = this.predictOccupancy(library, datetime);
    const basePricing = library.pricing || {
      hourly: 50,
      daily: 300
    };
  ...
  predictUserBehavior(userHistory) {
    ...
    // Find preferences
    const favoriteHour = Object.keys(hourPreferences).reduce((a, b) => hourPreferences[a] > hourPreferences[b] ? a : b);
    const favoriteDay = Object.keys(dayPreferences).reduce((a, b) => dayPreferences[a] > dayPreferences[b] ? a : b);
    const favoriteLibrary = Object.keys(libraryPreferences).reduce((a, b) => libraryPreferences[a] > libraryPreferences[b] ? a : b);
    const avgDuration = durationPreferences.length > 0 ? durationPreferences.reduce((a, b) => a + b, 0) / durationPreferences.length : 8;

    // Determine user type
    const bookingFrequency = userHistory.length;
    let userType = 'casual';
    if (bookingFrequency > 20) userType = 'power_user';else if (bookingFrequency > 10) userType = 'regular';else if (bookingFrequency > 5) userType = 'frequent';
    return {
      userType,
      pattern: 'established',
  ...
  predictNextBooking(userHistory) {
    ...
    const avgInterval = intervals.length > 0 ? intervals.reduce((a, b) => a + b, 0) / intervals.length : 7;
