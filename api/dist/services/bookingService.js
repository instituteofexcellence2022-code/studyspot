...
class BookingService {
  ...
  async createBooking(userId, bookingData) {
    try {
      ...
      // Check availability
      const isAvailable = await bookingRepository.checkAvailability(libraryId, seatId, bookingDate, startTime, endTime);
      if (!isAvailable) {
  ...
  async cancelBooking(bookingId, userId, reason) {
    try {
      ...
      if (refundAmount > 0 && booking.payment_status === 'completed') {
        await paymentService.processRefund(booking.razorpay_payment_id, refundAmount, 'Booking cancellation');
      }
