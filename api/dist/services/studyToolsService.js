...
class StudyToolsService {
  ...
  async createStudySession(userId, sessionData) {
    try {
      ...
      `, [userId, bookingId || null, type, JSON.stringify(settings), 'active']);
  ...
  async resumeTimer(userId, timerId) {
    try {
      ...
      if (result.rows.length === 0) {
        return {
          success: false,
          error: 'Timer not found or not paused'
        };
      }
      return {
        success: true,
  ...
  async calculateSessionStats(sessionId) {
    try {
      ...
      return {
        ...
        completionRate: stats.total_timers > 0 ? (parseInt(stats.completed_timers) / parseInt(stats.total_timers) * 100).toFixed(2) : 0
      };
    } catch (error) {
      ...
      return {
        ...
        data: {
          summary: {
            ...
            completionRate: stats.total_sessions > 0 ? (parseInt(stats.completed_sessions) / parseInt(stats.total_sessions) * 100).toFixed(2) : 0
          },
