...
async function logAudit(auditData) {
  try {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [event_type, user_id || null, target_user_id || null, target_resource_type || null, target_resource_id || null, action, description, ip_address || null, user_agent || null, metadata ? JSON.stringify(metadata) : null, severity, library_id || null, tenant_id || null]);
...
async function getSecurityEvents(filters = {}) {
  try {
    const securityEventTypes = [AUDIT_EVENTS.LOGIN_FAILED, AUDIT_EVENTS.UNAUTHORIZED_ACCESS, AUDIT_EVENTS.PERMISSION_DENIED, AUDIT_EVENTS.SUSPICIOUS_ACTIVITY];
...
async function createSession(sessionData) {
  try {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [user_id, session_token, ip_address, user_agent, expires_at]);
    return {
      success: true,
      data: result.rows[0]
    };
