...
class RecommendationService {
  ...
  getPersonalizedRecommendations(user, libraries, userBookings = [], options = {}) {
    ...
    const scoredLibraries = libraries.map(library => {
      ...
      const explanation = includeExplanation ? this.generateExplanation(user, library, score) : null;
      return {
        ...library,
    ...
    const recommendations = scoredLibraries.filter(lib => lib.recommendationScore >= minScore).sort((a, b) => b.recommendationScore - a.recommendationScore).slice(0, limit);
  ...
  amenitiesScore(user, library) {
    ...
    const matchedAmenities = userPreferences.filter(pref => libraryAmenities.some(amenity => amenity.toLowerCase().includes(pref.toLowerCase())));
  ...
  historyScore(user, library, userBookings) {
    ...
    const similarBookings = userBookings.filter(b => {
      ...
      return bookingLib.city === library.city || Math.abs((bookingLib.pricing?.daily || 0) - (library.pricing?.daily || 0)) < 100;
    });
  ...
  getMatchReasons(user, library, userBookings) {
    ...
    const userPreferences = user.preferredAmenities || [];
    if (userPreferences.length > 0) {
      const matches = userPreferences.filter(pref => library.amenities?.some(a => a.toLowerCase().includes(pref.toLowerCase())));
      if (matches.length > 0) {
  ...
  getTrendingLibraries(libraries, recentBookings, limit = 5) {
    ...
    const libraryCounts = {};
    recentBookings.filter(b => new Date(b.createdAt) > weekAgo).forEach(booking => {
  ...
  getSimilarLibraries(targetLibrary, allLibraries, limit = 5) {
    const similarities = allLibraries.filter(lib => lib.id !== targetLibrary.id).map(lib => ({
      ...lib,
