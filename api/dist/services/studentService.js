...
async function createStudent(studentData) {
  ...
  try {
    ...
    const insertQuery = `
      ...
      RETURNING *
    `;
    const result = await client.query(insertQuery, [name, email, phone, password,
    // Should be hashed in the route handler
    library_id, student_id, kyc_documents ? JSON.stringify(kyc_documents) : null]);
    const user = result.rows[0];

    // Add to groups if specified
    if (group_ids.length > 0) {
      for (const group_id of group_ids) {
        await client.query('INSERT INTO student_group_members (group_id, user_id) VALUES ($1, $2)', [group_id, user.id]);
      }
    }
  ...
  try {
    ...
    for (const studentData of studentsData) {
      try {
        ...
        `;
        const result = await client.query(insertQuery, [name, email, phone, password, libraryId, student_id]);
...
async function verifyKYC(studentId, kycData) {
  try {
    ...
    const query = `
      ...
      RETURNING *
    `;
    const result = await pool.query(query, [verified, documents ? JSON.stringify(documents) : null, verified_by, verified ? new Date() : null, notes, studentId]);
    if (result.rows.length === 0) {
      return {
        success: false,
        errors: [{
...
async function addStudentsToGroup(groupId, studentIds) {
  ...
  try {
    ...
    for (const studentId of studentIds) {
      await client.query(`INSERT INTO student_group_members (group_id, user_id) 
         VALUES ($1, $2) 
         ON CONFLICT DO NOTHING`, [groupId, studentId]);
    }
    await client.query('COMMIT');
    return {
      success: true,
