...
class MessagingService {
  ...
  async sendSMSTemplate(to, templateName, variables = {}) {
    try {
      ...
      const result = await query('SELECT content FROM message_templates WHERE name = $1 AND type = $2', [templateName, 'sms']);
  ...
  async sendWhatsAppTemplate(to, templateName, variables = {}) {
    try {
      ...
      const result = await query('SELECT content FROM message_templates WHERE name = $1 AND type = $2', [templateName, 'whatsapp']);
      if (result.rows.length === 0) {
        return {
          success: false,
          error: 'Template not found'
  ...
  async sendEmailTemplate(to, templateName, variables = {}) {
    try {
      ...
      const result = await query('SELECT subject, content FROM message_templates WHERE name = $1 AND type = $2', [templateName, 'email']);
      if (result.rows.length === 0) {
        return {
          success: false,
          error: 'Template not found'
  ...
  async sendMultiChannel(channels, recipient, message, options = {}) {
    ...
    try {
      ...
      if (channels.includes('email') && recipient.email) {
        results.email = await this.sendEmail(recipient.email, message.subject || 'Notification', message.html || message.text);
      }
  ...
  async deductCredits(tenantId, type, amount) {
    try {
      ...
      `, [tenantId, type, -amount, result.rows[0][`${type}_credits`], `Used ${amount} ${type} credits`]);
      return {
        success: true,
        newBalance: result.rows[0][`${type}_credits`]
      };
    } catch (error) {
  ...
  async checkCredits(tenantId, type, required) {
    try {
      const result = await query(`SELECT ${type}_credits FROM credit_balances WHERE tenant_id = $1`, [tenantId]);
      if (result.rows.length === 0) {
        return {
          success: false,
          hasEnough: false
