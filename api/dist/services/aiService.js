...
class AIService {
  ...
  async getUserPerformance(userId) {
    try {
      ...
      return {
        ...
        completionRate: metrics.total_sessions > 0 ? (parseInt(metrics.completed_sessions) / parseInt(metrics.total_sessions) * 100).toFixed(2) : 0
      };
    } catch (error) {
  ...
  recommendStudyDuration(performance) {
    ...
    return {
      duration: Math.round(recommendedDuration * 2) / 2,
  ...
  calculateLibraryRecommendationScore(library, visitedLibraries) {
    ...
    if (library.booking_count > 100) score += 20;else if (library.booking_count > 50) score += 10;

    // Increase score for high completion rate
    if (library.completion_rate > 0.8) score += 20;else if (library.completion_rate > 0.6) score += 10;
  ...
  async predictSuccessProbability(userId, bookingDetails) {
    try {
      ...
      const isPreferredTime = patterns.preferredTimeSlots?.some(slot => Math.abs(slot.hour - bookingHour) <= 1);
      ...
      const isPreferredDay = patterns.preferredDays?.some(day => day.day === ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][bookingDay]);
