...
class IoTService {
  ...
  async registerDevice(libraryId, deviceData) {
    try {
      ...
      const result = await query(`
        ...
        RETURNING *
      `, [libraryId, deviceName, deviceType, deviceId, location, 'active', false, JSON.stringify(configuration)]);
  ...
  async createAutomationRule(libraryId, ruleData) {
    try {
      ...
      const result = await query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING *
      `, [libraryId, name, description, triggerType, JSON.stringify(triggerConditions), JSON.stringify(actions), true, createdBy]);
  ...
  activateAutomationRule(rule) {
    ...
    logger.info('Automation rule activated', {
      ruleId: rule.id
    });
  }
  ...
  async recordEnergyConsumption(libraryId, consumptionData) {
    try {
      ...
      const result = await query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        RETURNING *
      `, [libraryId, deviceId, consumptionKwh, costAmount, currency, readingDate, readingTime, JSON.stringify(metadata)]);
      return {
        success: true,
        data: result.rows[0]
      };
