...
const createReferralProgram = asyncHandler(async (req, res) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const values = [uuidv4(), req.user.tenant_id, name, description, referral_code, referrer_bonus_type, referrer_bonus_value, referee_bonus_type, referee_bonus_value, min_referral_amount, max_referral_amount, validity_days, max_referrals_per_user, start_date, end_date, terms_and_conditions];
...
const createReferral = asyncHandler(async (req, res) => {
  ...
  const values = [uuidv4(), req.user.tenant_id, program.id, req.user.id, referral_code, referee_email, referee_phone, expiresAt];
...
const createDiscountCoupon = asyncHandler(async (req, res) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const values = [uuidv4(), req.user.tenant_id, code, name, description, discount_type, discount_value, min_order_amount, max_discount_amount, usage_limit, usage_limit_per_user, applicable_to, applicable_libraries, applicable_services, start_date, end_date, terms_and_conditions, req.user.id];
  const result = await pool.query(query, values);
  res.status(201).json({
    success: true,
    data: result.rows[0]
...
const applyCoupon = asyncHandler(async (req, res) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const values = [uuidv4(), req.user.tenant_id, coupon_id, user_id, booking_id, payment_id, discount_amount, original_amount, finalAmount];
  const result = await pool.query(query, values);
  res.status(201).json({
    success: true,
    data: result.rows[0]
...
const createPromotionalCampaign = asyncHandler(async (req, res) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const values = [uuidv4(), req.user.tenant_id, name, description, campaign_type, target_audience, JSON.stringify(target_segment), budget, expected_reach, expected_conversions, start_date, end_date, req.user.id];
  const result = await pool.query(query, values);
  res.status(201).json({
    success: true,
    data: result.rows[0]
...
const updateCampaignPerformance = asyncHandler(async (req, res) => {
  ...
  const query = `
    ...
    RETURNING *
  `;
  const values = [uuidv4(), req.user.tenant_id, campaignId, date, impressions, clicks, conversions, revenue, cost];
  const result = await pool.query(query, values);
  res.status(200).json({
    success: true,
    data: result.rows[0]
