...
router.get('/availability', [queryValidator('libraryId').isUUID().withMessage('Valid library ID is required'), queryValidator('date').isISO8601().withMessage('Valid date is required'), queryValidator('startTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Valid start time is required'), queryValidator('endTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Valid end time is required'), queryValidator('zone').optional().isString()], asyncHandler(async (req, res) => {
...
// Create booking
router.post('/', [body('libraryId').isUUID().withMessage('Valid library ID is required'), body('seatId').isUUID().withMessage('Valid seat ID is required'), body('date').isISO8601().withMessage('Valid date is required'), body('startTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Valid start time is required'), body('endTime').matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).withMessage('Valid end time is required'), body('bookingType').optional().isIn(['hourly', 'daily', 'monthly']).withMessage('Invalid booking type'), body('paymentMethod').optional().isIn(['online', 'offline']).withMessage('Invalid payment method')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
  ...
  `, [req.user.id, libraryId, seatId, date, startTime, endTime, bookingType, 'confirmed', paymentMethod === 'online' ? 'pending' : 'completed', totalAmount, paymentMethod, qrCode]);
...
router.delete('/:bookingId', asyncHandler(async (req, res) => {
  ...
  await query(`
    ...
    VALUES ($1, $2, $3, $4, $5)
  `, [req.user.id, 'Booking Cancelled', `Your booking for ${booking.library_name} has been cancelled.`, 'booking', JSON.stringify({
    bookingId,
...
// Check-in to library
router.post('/:bookingId/checkin', [body('qrCode').optional().isString(), body('location').optional().isObject()], asyncHandler(async (req, res) => {
  const {
    bookingId
  } = req.params;
  const {
  ...
  await query(`
    INSERT INTO notifications (user_id, title, message, type, data)
    VALUES ($1, $2, $3, $4, $5)
  `, [req.user.id, 'Check-in Successful', `You have successfully checked in to ${booking.library_name} for seat ${booking.seat_number}.`, 'booking', JSON.stringify({
    bookingId,
    libraryName: booking.library_name,
...
router.post('/:bookingId/checkout', asyncHandler(async (req, res) => {
  ...
  await query(`
    INSERT INTO notifications (user_id, title, message, type, data)
    VALUES ($1, $2, $3, $4, $5)
  `, [req.user.id, 'Check-out Successful', `You have successfully checked out from ${booking.library_name}. Thank you for using StudySpot!`, 'booking', JSON.stringify({
    bookingId,
    libraryName: booking.library_name,
    seatNumber: booking.seat_number
  })]);
