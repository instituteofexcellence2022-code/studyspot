...
router.post('/track', [queryValidator('eventType').isString().withMessage('Event type is required'), queryValidator('eventData').optional().isObject().withMessage('Event data must be an object')], asyncHandler(async (req, res) => {
  ...
  const result = await analyticsService.trackEvent(req.user.id, eventType, eventData, metadata);
  if (!result.success) {
...
router.get('/bookings', authorize(['super_admin', 'library_staff']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format'), queryValidator('groupBy').optional().isIn(['hour', 'day', 'week', 'month']).withMessage('Invalid groupBy value')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/payments', authorize(['super_admin', 'library_staff']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format'), queryValidator('groupBy').optional().isIn(['hour', 'day', 'week', 'month']).withMessage('Invalid groupBy value')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/users', authorize(['super_admin']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/libraries', authorize(['super_admin', 'library_staff']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/predictive-trends', authorize(['super_admin', 'library_staff']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/anomaly-detection', authorize(['super_admin', 'library_staff']), [queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
// Get cohort analysis
router.get('/cohort-analysis', authorize(['super_admin']), [queryValidator('cohortType').optional().isIn(['daily', 'weekly', 'monthly']).withMessage('Invalid cohort type')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
