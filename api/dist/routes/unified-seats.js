 ...
 */
router.post('/', authorize('super_admin', 'library_owner', 'branch_manager'), [body('libraryId').isUUID().withMessage('Valid library ID is required'), body('seatNumber').trim().isLength({
  min: 1
}).withMessage('Seat number is required'), body('seatType').optional().isIn(['standard', 'premium', 'vip', 'group', 'study']), body('status').optional().isIn(['active', 'inactive', 'maintenance']), body('capacity').optional().isInt({
  min: 1
}).withMessage('Capacity must be at least 1'), body('pricePerHour').optional().isFloat({
  min: 0
}).withMessage('Price must be non-negative')], asyncHandler(async (req, res) => {
 ...
 * - pricePerHour: Price per hour (optional)
 */
router.put('/:seatId', authorize('super_admin', 'library_owner', 'branch_manager'), [body('status').optional().isIn(['active', 'inactive', 'maintenance']), body('capacity').optional().isInt({
  min: 1
}), body('pricePerHour').optional().isFloat({
  min: 0
 ...
 */
router.delete('/:seatId', authorize('super_admin', 'library_owner'), unifiedDeleteSeat);
