 ...
 */
router.get('/balance/:type', authenticate, [param('type').isIn(['sms', 'whatsapp', 'email'])], validate, async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    const {
      type
 ...
 */
router.post('/purchase', authenticate, [body('packageId').isUUID().withMessage('Valid package ID required'), body('paymentMethod').isIn(['stripe', 'razorpay', 'manual']).withMessage('Valid payment method required')], validate, async (req, res) => {
  try {
    const {
      packageId,
      paymentMethod
    ...
    const purchaseResult = await dbQuery(`
        ...
        RETURNING *
      `, [tenantId, packageId, pkg.credit_amount, pkg.bonus_credits, pkg.price, pkg.currency, paymentMethod, userId]);
 ...
 */
router.get('/purchases', authenticate, [query('page').optional().isInt({
  min: 1
 ...
 */
router.get('/transactions', authenticate, [query('page').optional().isInt({
  min: 1
}), query('limit').optional().isInt({
  min: 1,
  max: 100
}), query('type').optional().isIn(['sms', 'whatsapp', 'email'])], validate, async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    const page = parseInt(req.query.page) || 1;
    ...
    // Get total count
    const countResult = await dbQuery(`SELECT COUNT(*) FROM credit_transactions ${whereClause}`, params);
    const total = parseInt(countResult.rows[0].count);
 ...
 */
router.post('/use', authenticate, [body('type').isIn(['sms', 'whatsapp', 'email']).withMessage('Valid type required'), body('amount').isInt({
  min: 1
}).withMessage('Amount must be positive integer'), body('referenceId').optional().isUUID(), body('description').optional().isString()], validate, async (req, res) => {
  try {
    const {
      type,
      amount,
 ...
 */
router.get('/usage', authenticate, [query('page').optional().isInt({
  min: 1
}), query('limit').optional().isInt({
  min: 1,
  max: 100
  ...
  try {
    ...
    // Get total count
    const countResult = await dbQuery(`SELECT COUNT(*) FROM credit_usage_logs ${whereClause}`, params);
    const total = parseInt(countResult.rows[0].count);
 ...
 */
router.get('/auto-topup', authenticate, async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    const result = await dbQuery(`
        SELECT *
 ...
 */
router.post('/auto-topup', authenticate, [body('creditType').isIn(['sms', 'whatsapp', 'email']), body('isEnabled').isBoolean(), body('thresholdAmount').isInt({
  min: 0
}), body('topupPackageId').isUUID(), body('paymentMethod').isIn(['stripe', 'razorpay'])], validate, async (req, res) => {
  try {
    const {
      creditType,
      isEnabled,
    ...
    const result = await dbQuery(`
        ...
        ON CONFLICT (tenant_id, credit_type) DO UPDATE SET
          ...
          updated_at = NOW()
        RETURNING *
      `, [tenantId, creditType, isEnabled, thresholdAmount, topupPackageId, pkg.credit_amount, paymentMethod]);
 ...
 * Disable auto-topup
 */
router.delete('/auto-topup/:type', authenticate, [param('type').isIn(['sms', 'whatsapp', 'email'])], validate, async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    const {
      type
