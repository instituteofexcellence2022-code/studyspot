...
router.get('/', optionalAuth, asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: libraries.rows.map(library => ({
      ...
      ...(library.distance && {
        distance: Math.round(library.distance / 1000 * 100) / 100
      }),
      // Convert to km
...
router.post('/', [verifyToken, authorize('super_admin', 'branch_manager'), body('name').trim().isLength({
  min: 1
}).withMessage('Library name is required'), body('description').optional().trim(), body('address').trim().isLength({
  min: 1
}).withMessage('Address is required'), body('city').trim().isLength({
  min: 1
}).withMessage('City is required'), body('state').trim().isLength({
  min: 1
}).withMessage('State is required'), body('country').optional().trim().default('India'), body('postalCode').optional().trim(), body('latitude').isFloat({
  min: -90,
  max: 90
}).withMessage('Invalid latitude'), body('longitude').isFloat({
  min: -180,
  max: 180
}).withMessage('Invalid longitude'), body('capacity').isInt({
  min: 1
}).withMessage('Capacity must be a positive integer'), body('amenities').optional().isArray().withMessage('Amenities must be an array'), body('pricing').optional().isObject().withMessage('Pricing must be an object'), body('operatingHours').optional().isObject().withMessage('Operating hours must be an object'), body('contactInfo').optional().isObject().withMessage('Contact info must be an object')], asyncHandler(async (req, res) => {
  ...
  `, [req.user.tenantId, name, description, address, city, state, country, postalCode, latitude, longitude, capacity, JSON.stringify(amenities || []), JSON.stringify(pricing || {}), JSON.stringify(operatingHours || {}), JSON.stringify(contactInfo || {})]);
  const library = libraryResult.rows[0];
...
router.put('/:libraryId', [verifyToken, authorize('super_admin', 'branch_manager'), body('name').optional().trim().isLength({
  min: 1
}).withMessage('Library name cannot be empty'), body('description').optional().trim(), body('address').optional().trim().isLength({
  min: 1
}).withMessage('Address cannot be empty'), body('city').optional().trim().isLength({
  min: 1
}).withMessage('City cannot be empty'), body('state').optional().trim().isLength({
  min: 1
}).withMessage('State cannot be empty'), body('latitude').optional().isFloat({
  min: -90,
  max: 90
}).withMessage('Invalid latitude'), body('longitude').optional().isFloat({
  min: -180,
  max: 180
}).withMessage('Invalid longitude'), body('capacity').optional().isInt({
  min: 1
}).withMessage('Capacity must be a positive integer'), body('amenities').optional().isArray().withMessage('Amenities must be an array'), body('pricing').optional().isObject().withMessage('Pricing must be an object'), body('operatingHours').optional().isObject().withMessage('Operating hours must be an object'), body('contactInfo').optional().isObject().withMessage('Contact info must be an object')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
  ...
  const allowedFields = ['name', 'description', 'address', 'city', 'state', 'country', 'postal_code', 'latitude', 'longitude', 'capacity', 'amenities', 'pricing', 'operating_hours', 'contact_info'];
...
router.delete('/:libraryId', [verifyToken, authorize('super_admin', 'branch_manager')], asyncHandler(async (req, res) => {
  const {
    libraryId
  } = req.params;
