...
router.get('/', auth, [query('page').optional().isInt({
  min: 1
}), query('limit').optional().isInt({
  min: 1,
  max: 100
}), query('status').optional().isString(), query('priority').optional().isString(), query('category').optional().isString(), query('assigned_to').optional().isUUID(), query('library_id').optional().isUUID(), query('search').optional().isString(), query('sort_by').optional().isIn(['created_at', 'updated_at', 'priority', 'status']), query('sort_order').optional().isIn(['asc', 'desc'])], asyncHandler(async (req, res) => {
...
router.post('/', auth, [body('title').trim().isLength({
  min: 1,
  max: 255
}).withMessage('Title is required'), body('description').trim().isLength({
  min: 1
}).withMessage('Description is required'), body('category_id').isUUID().withMessage('Valid category ID is required'), body('priority_id').isUUID().withMessage('Valid priority ID is required'), body('library_id').optional().isUUID(), body('reported_by_name').optional().isString(), body('reported_by_email').optional().isEmail(), body('reported_by_phone').optional().isString(), body('student_count').optional().isInt({
  min: 1
}), body('tags').optional().isArray(), body('attachments').optional().isArray()], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
  ...
  const priorityResult = await dbQuery('SELECT level FROM issue_priorities WHERE id = $1', [priority_id]);
  ...
  const statusResult = await dbQuery("SELECT id FROM issue_statuses WHERE name = 'open'");
  ...
  `, [tenantId, library_id, title, description, category_id, priority_id, statusId, req.user.id, reported_by_name, reported_by_email, reported_by_phone, student_count, slaDeadline, tags, JSON.stringify(attachments), reportedAt]);
  ...
  res.status(201).json({
    ...
    data: {
      ...
      message: duplicates.length > 0 ? 'Issue created. Similar issues found - consider merging.' : 'Issue created successfully'
    },
    meta: {
      timestamp: new Date().toISOString()
    }
...
// Update issue
router.put('/:id', auth, [body('title').optional().trim().isLength({
  min: 1,
  max: 255
}), body('description').optional().trim().isLength({
  min: 1
}), body('category_id').optional().isUUID(), body('priority_id').optional().isUUID(), body('status_id').optional().isUUID(), body('assigned_to_user_id').optional().isUUID(), body('student_count').optional().isInt({
  min: 1
}), body('tags').optional().isArray(), body('attachments').optional().isArray(), body('satisfaction_rating').optional().isInt({
  min: 1,
  max: 5
}), body('satisfaction_feedback').optional().isString()], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
  ...
  const existingIssue = await dbQuery('SELECT * FROM issues WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
  ...
  if (updateData.status_id) {
    const statusResult = await dbQuery('SELECT name, is_final FROM issue_statuses WHERE id = $1', [updateData.status_id]);
    if (statusResult.rows.length > 0) {
...
router.delete('/:id', auth, asyncHandler(async (req, res) => {
  ...
  const tenantId = req.user.tenantId;
  const result = await dbQuery('DELETE FROM issues WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, tenantId]);
  if (result.rows.length === 0) {
    throw new AppError('Issue not found', 404, 'ISSUE_NOT_FOUND');
  }
...
router.get('/:id/comments', auth, asyncHandler(async (req, res) => {
  ...
  const issueCheck = await dbQuery('SELECT id FROM issues WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
...
router.post('/:id/comments', auth, [body('comment').trim().isLength({
  min: 1
}).withMessage('Comment is required'), body('is_internal').optional().isBoolean(), body('attachments').optional().isArray()], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
  ...
  await dbQuery('UPDATE issues SET updated_at = NOW() WHERE id = $1', [id]);
  res.status(201).json({
    success: true,
    data: result.rows[0],
    meta: {
...
router.post('/templates', auth, [body('name').trim().isLength({
  min: 1,
  max: 255
}).withMessage('Name is required'), body('subject').trim().isLength({
  min: 1,
  max: 255
}).withMessage('Subject is required'), body('body').trim().isLength({
  min: 1
}).withMessage('Body is required'), body('category_id').optional().isUUID(), body('priority_id').optional().isUUID()], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.post('/duplicates', auth, [body('title').trim().isLength({
  min: 1
}).withMessage('Title is required'), body('description').trim().isLength({
  min: 1
}).withMessage('Description is required'), body('library_id').optional().isUUID()], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.post('/:id/merge', auth, [body('duplicate_issue_id').isUUID().withMessage('Valid duplicate issue ID is required')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
