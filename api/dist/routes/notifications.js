...
router.get('/', [queryValidator('page').optional().isInt({
  min: 1
}).withMessage('Page must be a positive integer'), queryValidator('limit').optional().isInt({
  min: 1,
  max: 100
}).withMessage('Limit must be between 1 and 100'), queryValidator('type').optional().isString().withMessage('Type must be a string'), queryValidator('isRead').optional().isBoolean().withMessage('isRead must be a boolean')], asyncHandler(async (req, res) => {
...
router.put('/preferences', [body('preferences').isObject().withMessage('Preferences must be an object'), body('preferences.in_app').optional().isBoolean().withMessage('in_app must be a boolean'), body('preferences.email').optional().isBoolean().withMessage('email must be a boolean'), body('preferences.sms').optional().isBoolean().withMessage('sms must be a boolean'), body('preferences.push').optional().isBoolean().withMessage('push must be a boolean')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.post('/test', [body('channels').isArray().withMessage('Channels must be an array'), body('channels.*').isIn(['in_app', 'email', 'sms']).withMessage('Invalid channel'), body('type').optional().isString().withMessage('Type must be a string')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
