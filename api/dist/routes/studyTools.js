...
router.post('/sessions', [body('bookingId').optional().isUUID().withMessage('Valid booking ID required'), body('type').optional().isIn(['pomodoro', 'custom', 'flexible']).withMessage('Invalid session type'), body('customSettings').optional().isObject().withMessage('Custom settings must be an object')], asyncHandler(async (req, res) => {
...
// Get session history
router.get('/sessions', [queryValidator('limit').optional().isInt({
  min: 1,
  max: 100
}).withMessage('Limit must be between 1 and 100'), queryValidator('offset').optional().isInt({
  min: 0
}).withMessage('Offset must be positive'), queryValidator('dateFrom').optional().isISO8601().withMessage('Invalid date format'), queryValidator('dateTo').optional().isISO8601().withMessage('Invalid date format')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.post('/timers', [body('sessionId').isUUID().withMessage('Valid session ID required'), body('type').isIn(['focus', 'break', 'short_break', 'long_break']).withMessage('Invalid timer type'), body('duration').isInt({
  min: 1,
  max: 240
}).withMessage('Duration must be between 1 and 240 minutes')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
...
router.get('/statistics', [queryValidator('period').optional().isIn(['7days', '30days', '90days']).withMessage('Invalid period')], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
      details: errors.array()
