...
router.post('/stripe', express.raw({
  ...
  try {
    ...
    const eventResult = stripeService.constructWebhookEvent(req.body, signature, webhookSecret);
...
async function handleSubscriptionCreated(subscription) {
  try {
    ...
    `, [subscription.status, new Date(subscription.current_period_start * 1000), new Date(subscription.current_period_end * 1000), subscription.id]);
...
async function handleSubscriptionUpdated(subscription) {
  try {
    ...
    `, [subscription.status, new Date(subscription.current_period_start * 1000), new Date(subscription.current_period_end * 1000), subscription.cancel_at_period_end, subscription.id]);
...
async function handleSubscriptionDeleted(subscription) {
  try {
    ...
    const subResult = await query('SELECT tenant_id FROM subscriptions WHERE stripe_subscription_id = $1', [subscription.id]);
...
async function handleInvoicePaid(invoice) {
  try {
    ...
    // Get subscription
    const subResult = await query('SELECT tenant_id FROM subscriptions WHERE stripe_subscription_id = $1', [invoice.subscription]);
    if (subResult.rows.length === 0) {
    ...
    await query(`
      ...
      DO UPDATE SET
        ...
        paid_at = $6,
        updated_at = NOW()
    `, [tenantId, invoice.number, invoice.amount_due / 100,
    // Convert from cents
    invoice.total / 100, 'paid', new Date(invoice.status_transitions.paid_at * 1000), invoice.id, invoice.invoice_pdf]);
