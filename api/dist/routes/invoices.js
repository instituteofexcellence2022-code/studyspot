 ...
 */
router.post('/', authenticate, requirePermission('invoices:generate'), async (req, res) => {
  try {
    const {
      user_id,
      payment_id,
 ...
 */
router.get('/revenue-analytics', authenticate, requirePermission('payments:view_revenue'), restrictToOwnLibrary, async (req, res) => {
  try {
    const filters = {
      ...req.libraryFilter,
 ...
 * Record an expense
 */
router.post('/expenses', authenticate, requirePermission('expenses:manage'), async (req, res) => {
  try {
    const {
      category,
      description,
 ...
 */
router.get('/expense-analytics', authenticate, requirePermission('expenses:manage'), restrictToOwnLibrary, async (req, res) => {
  try {
    const filters = {
      ...req.libraryFilter,
      start_date: req.query.start_date,
 ...
 */
router.get('/profit-loss', authenticate, requirePermission('payments:view_revenue'), restrictToOwnLibrary, async (req, res) => {
  try {
    const filters = {
      ...req.libraryFilter,
      start_date: req.query.start_date,
 ...
 */
router.get('/calculate-gst', authenticate, async (req, res) => {
  try {
    const {
      amount,
      library_state,
    ...
    const gstDetails = paymentService.calculateGST(parseFloat(amount), library_state, customer_state, gst_rate ? parseFloat(gst_rate) : undefined);
    res.json({
      success: true,
      data: {
