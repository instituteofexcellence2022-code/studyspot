...
async function seedPhase6() {
  ...
  try {
    ...
    for (const plan of plans) {
      await client.query(`INSERT INTO subscription_plans (name, display_name, description, price_monthly, price_yearly, features, limits)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (name) DO NOTHING`, [plan.name, plan.display_name, plan.description, plan.price_monthly, plan.price_yearly, plan.features, plan.limits]);
    }
    ...
    // Library permissions
    {
      ...
      resource: 'libraries',
      action: 'create',
      description: 'Create new library locations'
    }, {
      name: 'libraries.read',
      display_name: 'View Libraries',
      resource: 'libraries',
      action: 'read',
      description: 'View library details'
    }, {
      name: 'libraries.update',
      display_name: 'Update Libraries',
      resource: 'libraries',
      action: 'update',
      description: 'Edit library information'
    }, {
      name: 'libraries.delete',
      display_name: 'Delete Libraries',
      resource: 'libraries',
      action: 'delete',
      description: 'Remove libraries'
    }, {
      name: 'libraries.manage',
      display_name: 'Manage Libraries',
      resource: 'libraries',
      action: 'manage',
    ...
    // Booking permissions
    {
      ...
      display_name: 'Manage All Bookings',
      resource: 'bookings',
      action: 'manage',
    ...
    {
      name: 'credits.read',
      display_name: 'View Credits',
      resource: 'credits',
      action: 'read',
      description: 'View credit balance'
    }, {
      name: 'credits.purchase',
      display_name: 'Purchase Credits',
      resource: 'credits',
      action: 'purchase',
      ...
      resource: 'credits',
      action: 'manage',
      description: 'Full credit management'
    }];
    for (const perm of permissions) {
      await client.query(`INSERT INTO permissions (name, display_name, description, resource, action)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (name) DO NOTHING`, [perm.name, perm.display_name, perm.description, perm.resource, perm.action]);
    }
    ...
    const roles = [{
      ...
      is_system: true,
      permissions: ['libraries.manage', 'bookings.manage', 'users.manage', 'settings.manage', 'analytics.read', 'analytics.export', 'finance.read', 'finance.manage', 'roles.read', 'roles.assign', 'subscriptions.read', 'subscriptions.manage', 'credits.manage']
    }, {
      name: 'branch_manager',
      display_name: 'Branch Manager',
      description: 'Manages a single library location',
      is_system: true,
      permissions: ['libraries.read', 'libraries.update', 'bookings.manage', 'users.read', 'users.create', 'analytics.read', 'settings.read', 'credits.read']
    }, {
      name: 'front_desk',
      display_name: 'Front Desk Staff',
      description: 'Handles bookings and customer service',
      is_system: true,
      permissions: ['bookings.create', 'bookings.read', 'bookings.update', 'users.read', 'users.create', 'libraries.read']
    }, {
      name: 'customer',
      display_name: 'Customer',
      description: 'Regular library user',
      is_system: true,
      permissions: ['bookings.create', 'bookings.read', 'bookings.update', 'bookings.delete', 'libraries.read', 'users.read']
    }];
    for (const role of roles) {
      // Insert role
      const roleResult = await client.query(`INSERT INTO roles (name, display_name, description, is_system, tenant_id)
         VALUES ($1, $2, $3, $4, NULL)
         ON CONFLICT (tenant_id, name) WHERE tenant_id IS NULL DO UPDATE SET display_name = EXCLUDED.display_name
         RETURNING id`, [role.name, role.display_name, role.description, role.is_system]);
    ...
    // WhatsApp packages
    {
      ...
      name: 'whatsapp-500',
      display_name: '500 WhatsApp Credits',
      credit_type: 'whatsapp',
      credit_amount: 500,
      price: 65.00,
      bonus_credits: 50
      ...
      name: 'whatsapp-1000',
      display_name: '1000 WhatsApp Credits',
      credit_type: 'whatsapp',
      credit_amount: 1000,
      price: 120.00,
      bonus_credits: 200
      ...
      name: 'whatsapp-5000',
      display_name: '5000 WhatsApp Credits',
      credit_type: 'whatsapp',
      credit_amount: 5000,
      price: 500.00,
      bonus_credits: 1500
    ...
    {
      name: 'email-1000',
      display_name: '1000 Email Credits',
      credit_type: 'email',
      credit_amount: 1000,
      price: 5.00,
      bonus_credits: 0
      ...
      name: 'email-5000',
      display_name: '5000 Email Credits',
      credit_type: 'email',
      credit_amount: 5000,
      price: 20.00,
      bonus_credits: 500
      ...
      name: 'email-10000',
      display_name: '10000 Email Credits',
      credit_type: 'email',
      credit_amount: 10000,
      price: 35.00,
      bonus_credits: 2000
    ...
    for (const pkg of creditPackages) {
      await client.query(`INSERT INTO credit_packages (name, display_name, credit_type, credit_amount, price, bonus_credits, currency)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT DO NOTHING`, [pkg.name, pkg.display_name, pkg.credit_type, pkg.credit_amount, pkg.price, pkg.bonus_credits, 'USD']);
    }
...
if (require.main === module) {
  seedPhase6().then(() => process.exit(0)).catch(error => {
    console.error(error);
    process.exit(1);
  });
}
