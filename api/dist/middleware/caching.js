...
const cacheMiddleware = (duration = 3600, keyGenerator = null) => {
  return async (req, res, next) => {
    ...
    try {
      ...
      // Generate cache key
      const cacheKey = keyGenerator ? keyGenerator(req) : `cache:${req.originalUrl}:${req.user?.id || 'anonymous'}`;
      ...
      res.json = data => {
        ...
        if (res.statusCode >= 200 && res.statusCode < 300) {
          redis.setex(cacheKey, duration, JSON.stringify(data)).catch(err => logger.error('Cache set error', err));
        }
