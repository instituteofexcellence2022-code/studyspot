...
const executePaginatedQuery = async (queryFn, baseQuery, params, pagination) => {
  ...
  const [dataResult, countResult] = await Promise.all([queryFn(query, params), queryFn(countQuery, params)]);
  return {
  ...
  return (req, res, next) => {
    try {
      ...
      req.cursorResponse = (data, hasMore) => {
        const lastCursor = data.length > 0 ? data[data.length - 1][config.cursorField] : null;
        return {
          success: true,
          data: data,
          meta: {
