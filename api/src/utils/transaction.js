...
const withTransaction = async (callback, options = {}) => {
  const {
    isolationLevel = null,  // 'READ COMMITTED', 'REPEATABLE READ', 'SERIALIZABLE'
    readOnly = false,
    deferrable = false
  } = options;
 ...
 * );
 */
const withStripeTransaction = async (
  stripeOperation,
  dbOperation,
  rollbackStripeOperation = null
) => {
  ...
  try {
    ...
    return { stripeResult, dbResult: result };
