...
const createIssueCategory = asyncHandler(async (req, res) => {
  const { name, display_name, description, icon, color, is_active = true } = req.body;
...
const createIssuePriority = asyncHandler(async (req, res) => {
  const { name, display_name, level, color, sla_hours, is_active = true } = req.body;
...
const createIssueStatus = asyncHandler(async (req, res) => {
  const { name, display_name, description, color, is_final = false, is_active = true } = req.body;
...
const updateIssue = asyncHandler(async (req, res) => {
  ...
  if (updates.status_id && updates.status_id !== oldIssue.status_id) {
    await triggerWorkflowAutomation('status_changed', updatedIssue, { old_status_id: oldIssue.status_id });
  }
  
  // Trigger workflow automation for assignment changes
  if (updates.assigned_to && updates.assigned_to !== oldIssue.assigned_to) {
    await triggerWorkflowAutomation('assignment_changed', updatedIssue, { old_assigned_to: oldIssue.assigned_to });
  }
...
const addIssueComment = asyncHandler(async (req, res) => {
  const { issueId } = req.params;
  const { comment, user_id, is_internal = false } = req.body;
...
const addIssueAttachment = asyncHandler(async (req, res) => {
  const { issueId } = req.params;
  const { file_name, file_url, uploaded_by, file_type, file_size } = req.body;
...
const unassignStaffFromIssue = asyncHandler(async (req, res) => {
  const { issueId, staffId } = req.params;
...
const createResponseTemplate = asyncHandler(async (req, res) => {
  const { name, subject, body, category_id, is_active = true } = req.body;
...
const getIssueSummaryAnalytics = asyncHandler(async (req, res) => {
  ...
  const analytics = {
    ...
    average_resolution_time: result.rows[0].avg_resolution_time_hours 
      ? `${Math.round(result.rows[0].avg_resolution_time_hours)} hours`
      : 'N/A',
