...
app.get('/api/offline-payments/history', (req, res) => {
  const { studentId, staffId, dateFrom, dateTo, status } = req.query;
...
app.get('/api/offline-payments/analytics', (req, res) => {
  ...
  const analytics = {
    ...
    paymentMethods: [
      ...
      { method: 'card', count: 8, amount: 4000 },
      { method: 'upi', count: 2, amount: 1000 }
    ],
    staffPerformance: [
      { staffName: 'John Doe', count: 15, amount: 7500 },
      { staffName: 'Jane Smith', count: 10, amount: 5000 },
      { staffName: 'Mike Johnson', count: 5, amount: 2500 }
    ]
  };
