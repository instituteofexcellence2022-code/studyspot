...
router.post('/distance', [
  body('origin.latitude').isFloat({ min: -90, max: 90 }).withMessage('Valid origin latitude is required'),
  body('origin.longitude').isFloat({ min: -180, max: 180 }).withMessage('Valid origin longitude is required'),
  body('destination.latitude').isFloat({ min: -90, max: 90 }).withMessage('Valid destination latitude is required'),
  body('destination.longitude').isFloat({ min: -180, max: 180 }).withMessage('Valid destination longitude is required')
], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
...
router.get('/nearby-libraries', [
  ...
  queryValidator('radius').optional().isFloat({ min: 0.1, max: 100 }).withMessage('Radius must be between 0.1 and 100 km')
], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    throw new AppError('Validation failed', 400, 'VALIDATION_ERROR', {
  ...
  const nearbyLibraries = await mapsService.findNearbyLibraries(
    parseFloat(latitude), 
...
router.post('/search-places', [
  ...
  body('radius').optional().isFloat({ min: 1, max: 50000 }).withMessage('Radius must be between 1 and 50000 meters'),
  ...
  const { query: searchQuery, location, radius = 5000, type } = req.body;

  const searchLocation = location ? { lat: location.latitude, lng: location.longitude } : null;
...
router.get('/place-details/:placeId', [
  queryValidator('fields').optional().isString().withMessage('Fields must be a string')
], asyncHandler(async (req, res) => {
...
router.post('/static-map', [
  body('latitude').isFloat({ min: -90, max: 90 }).withMessage('Valid latitude is required'),
  body('longitude').isFloat({ min: -180, max: 180 }).withMessage('Valid longitude is required'),
  body('zoom').optional().isInt({ min: 1, max: 20 }).withMessage('Zoom must be between 1 and 20'),
  body('size').optional().matches(/^\d+x\d+$/).withMessage('Size must be in format WIDTHxHEIGHT'),
  ...
  const { latitude, longitude, zoom = 15, size = '400x400', markers = [] } = req.body;
