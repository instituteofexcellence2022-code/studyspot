...
const validatePayment = [
  ...
  body('paymentMethod').isIn(['cash', 'cheque', 'bank_transfer', 'online', 'upi', 'card', 'net_banking']).withMessage('Invalid payment method'),
  body('description').notEmpty().trim().withMessage('Description is required'),
  body('receivedBy').notEmpty().trim().withMessage('Staff name (received by) is required'),
  body('staffConfirmation').isBoolean().withMessage('Staff confirmation must be boolean'),
  body('transactionDate').optional().isISO8601().withMessage('Invalid date format'),
];

const validateVerification = [
  body('verificationStatus').isIn(['approved', 'rejected', 'on_hold']).withMessage('Invalid verification status'),
...
router.get('/',
  ...
  [
    ...
    query('method').optional().isIn(['cash', 'cheque', 'bank_transfer', 'online', 'upi', 'card', 'net_banking']),
  ...
  async (req, res) => {
    try {
      ...
      logger.info('Payments fetched', {
        ...
        total,
        page,
        filters: { status, method, verified, search }
      });
  ...
  async (req, res) => {
    try {
      ...
      const queueValues = [
        payment.id,
        'pending',
        paymentMethod === 'cash' ? 1 : 2, // Cash gets higher priority
        receivedBy,
        staffId,
...
router.post('/:paymentId/verify',
  ...
  async (req, res) => {
    try {
      ...
      const paymentResult = await dbQuery(updatePaymentQuery, [
        isApproved,
        verifierId,
        new Date(),
      ...
      await dbQuery(updateQueueQuery, [
        verificationStatus,
        verifierId,
        new Date(),
