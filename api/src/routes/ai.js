...
router.get('/insights', asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: {
      ...
      patterns: patterns,
      insights: {
        consistencyLevel: patterns.consistencyScore >= 70 ? 'excellent' : 
                         patterns.consistencyScore >= 50 ? 'good' : 'needs improvement',
        productivityTrend: performance.completionRate >= 80 ? 'increasing' : 
                           performance.completionRate >= 60 ? 'stable' : 'declining',
        recommendations: patterns.consistencyScore < 70 ? 
          ['Try to maintain a regular study schedule', 'Book sessions at consistent times'] :
          ['Keep up the great work!', 'Consider challenging yourself with longer sessions']
      }
    },
    meta: {
      timestamp: new Date().toISOString(),
...
router.post('/predict-success', [
  ...
  body('bookingDetails.date').isISO8601().withMessage('Valid date is required'),
  body('bookingDetails.startTime').isString().withMessage('Start time is required'),
  body('bookingDetails.endTime').isString().withMessage('End time is required')
  ...
  res.json({
    ...
    data: {
      ...
      overallScore: Math.round((
        ...
        (performance.uniqueStudyDays / 30 * 100) +
        (Math.min(performance.averageSessionDuration / 3 * 100, 100))
      ) / 3),
...
async function generateAssistantResponse(message, userId) {
  ...
  if (lowerMessage.includes('recommend') || lowerMessage.includes('suggest')) {
    return {
      ...
      actions: [
        { label: 'Get Recommendations', endpoint: '/api/ai/recommendations' },
        { label: 'View Schedule', endpoint: '/api/ai/optimal-schedule' }
      ]
    };
  }
  ...
  if (lowerMessage.includes('improve') || lowerMessage.includes('better')) {
    return {
      ...
      actions: [
        { label: 'View Weak Areas', endpoint: '/api/ai/improvement-areas' },
        { label: 'Get Study Plan', endpoint: '/api/ai/optimal-schedule' }
      ]
    };
  }
  ...
  if (lowerMessage.includes('library') || lowerMessage.includes('where')) {
    return {
      ...
      actions: [
        { label: 'Recommended Libraries', endpoint: '/api/ai/recommended-libraries' },
        { label: 'Nearby Libraries', endpoint: '/api/maps/nearby-libraries' }
      ]
    };
  }
  ...
  return {
    ...
    suggestions: [
      ...
      'See recommended libraries',
      'Check my progress'
    ],
    actions: [
      { label: 'Recommendations', endpoint: '/api/ai/recommendations' },
      { label: 'Insights', endpoint: '/api/ai/insights' }
    ]
  };
}
