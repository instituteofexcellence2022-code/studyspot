...
router.post('/onboard',
  ...
  [
    ...
    body('domain').trim().isLength({ min: 3, max: 100 }).withMessage('Domain must be 3-100 characters'),
  ...
  async (req, res) => {
    try {
      ...
      const domainCheck = await dbQuery(
      ...
      const tenantResult = await dbQuery(`
        ...
        RETURNING *
      `, [
        name, domain, ownerName, ownerEmail, phone,
        businessType, address, city, state, country || 'India',
        'pending', 1, true
      ]);
      ...
      `, [
        ...
        hashedPassword,
        ownerName.split(' ')[0],
        ownerName.split(' ').slice(1).join(' ') || ownerName.split(' ')[0],
        phone,
        'library_owner',
        true,
        false
 ...
 */
router.put('/:id/onboarding-step',
  authenticate,
  requireTenantOwnerOrAdmin,
  [
    param('id').isUUID().withMessage('Valid tenant ID required'),
    body('step').isInt({ min: 1, max: 10 }).withMessage('Step must be between 1-10'),
 ...
 */
router.get('/:id/onboarding-status',
  authenticate,
  requireTenantOwnerOrAdmin,
  [param('id').isUUID()],
  validate,
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          pendingTasks: tenant.onboarding_step < 5 ? [
            'Complete profile',
            'Add library',
            'Configure settings',
            'Choose subscription',
            'Verify account'
          ].slice(tenant.onboarding_step - 1) : []
        }
      });
...
router.post('/:id/suspend',
  ...
  async (req, res) => {
    try {
      ...
      logger.info('Tenant suspended', { tenantId: id, reason });
