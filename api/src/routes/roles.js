...
router.get('/',
  ...
  async (req, res) => {
    try {
      const { tenantId, includeSystem } = req.query;
...
router.post('/',
  ...
  [
    ...
    body('displayName').trim().isLength({ min: 2, max: 255 }).withMessage('Display name required'),
    body('description').optional().isString(),
    body('permissionIds').isArray().withMessage('Permission IDs array required'),
  ],
  validate,
  async (req, res) => {
    try {
...
router.put('/:id',
  ...
  [
    param('id').isUUID(),
    body('displayName').optional().trim().isLength({ min: 2, max: 255 }),
    body('description').optional().isString(),
    ...
    try {
      ...
      if (permissionIds) {
        ...
        if (permissionIds.length > 0) {
          const permValues = permissionIds.map((permId, idx) => 
            `($1, $${idx + 2})`
          ).join(', ');
...
router.post('/:id/assign',
  ...
  [
    param('id').isUUID().withMessage('Valid role ID required'),
 ...
 */
router.delete('/:id/revoke/:userId',
  authenticate,
  requirePermission('roles.assign'),
  [
    param('id').isUUID(),
