...
const { formatPaginatedResponse, formatSingleResponse } = require('../utils/responseFormatter');
...
const unifiedGetSeats = asyncHandler(async (req, res) => {
  ...
  const { page: validatedPage, limit: validatedLimit } = validatePagination(page, limit || 50);
  ...
  if (available !== undefined) {
    const availabilityQuery = available === 'true'
      ? 'NOT EXISTS (SELECT 1 FROM bookings b WHERE b.seat_id = s.id AND b.status IN (\'confirmed\', \'pending\') AND b.booking_date = CURRENT_DATE)'
      : 'EXISTS (SELECT 1 FROM bookings b WHERE b.seat_id = s.id AND b.status IN (\'confirmed\', \'pending\') AND b.booking_date = CURRENT_DATE)';
  ...
  const meta = {
    pagination: { page: validatedPage, limit: validatedLimit, total, totalPages: Math.ceil(total / validatedLimit) },
    filters: { libraryId, zone, status, available, seatType }
  };

  const data = seats.rows.map(seat => ({
    id: seat.id,
    library: { id: seat.library_id, name: seat.library_name },
...
const unifiedCreateSeat = asyncHandler(async (req, res) => {
  const { libraryId, seatNumber, zone, seatType = 'standard', status = 'active', capacity = 1, features = {}, pricePerHour } = req.body;
  ...
  logger.logBusinessEvent('seat_created', { seatId: seat.id, libraryId, seatNumber, tenantId: req.user.tenantId });

  res.status(201).json(formatSingleResponse({
    id: seat.id, libraryId: seat.library_id, seatNumber: seat.seat_number, zone: seat.zone,
    seatType: seat.seat_type, status: seat.status, capacity: seat.capacity,
    features: JSON.parse(seat.features), pricePerHour: seat.price_per_hour, createdAt: seat.created_at
  }));
});

const unifiedUpdateSeat = asyncHandler(async (req, res) => {
  const { seatId } = req.params;
  const { status, zone, seatType, capacity, features, pricePerHour } = req.body;
  ...
  if (zone !== undefined) { updateFields.push(`zone = $${paramCount++}`); updateValues.push(zone); }
  ...
  if (features !== undefined) { updateFields.push(`features = $${paramCount++}`); updateValues.push(JSON.stringify(features)); }
  if (pricePerHour !== undefined) { updateFields.push(`price_per_hour = $${paramCount++}`); updateValues.push(pricePerHour); }
  ...
  const seat = result.rows[0];
  logger.logBusinessEvent('seat_updated', { seatId: seat.id, updatedFields: Object.keys(req.body), tenantId: req.user.tenantId });

  res.json(formatSingleResponse({
    id: seat.id, libraryId: seat.library_id, seatNumber: seat.seat_number, zone: seat.zone,
    seatType: seat.seat_type, status: seat.status, capacity: seat.capacity,
    features: JSON.parse(seat.features), pricePerHour: seat.price_per_hour, updatedAt: seat.updated_at
  }));
});
...
const unifiedDeleteSeat = asyncHandler(async (req, res) => {
  ...
  const seat = result.rows[0];
  logger.logBusinessEvent('seat_deleted', { seatId: seat.id, seatNumber: seat.seat_number, tenantId: req.user.tenantId });

  res.json(formatSingleResponse({ message: 'Seat deleted successfully', seatId: seat.id }));
});

module.exports = { unifiedGetSeats, unifiedCreateSeat, unifiedUpdateSeat, unifiedDeleteSeat };
