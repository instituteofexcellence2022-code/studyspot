...
const { validatePagination, validateEnum, validateRequired, validateEmail } = require('../utils/validators');
...
const unifiedGetUsers = asyncHandler(async (req, res) => {
  ...
  if (search) {
    queryBuilder.whereCustom(
      '(u.email ILIKE ? OR u.first_name ILIKE ? OR u.last_name ILIKE ? OR u.phone ILIKE ?)',
...
const unifiedCreateUser = asyncHandler(async (req, res) => {
  ...
  validateRequired({ email, firstName, lastName }, ['email', 'firstName', 'lastName']);
