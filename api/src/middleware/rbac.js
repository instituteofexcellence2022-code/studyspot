...
const { hasPermission, getRolePermissions, canManageRole, ROLES } = require('../constants/roles');
...
const restrictToOwnLibrary = (req, res, next) => {
  try {
    ...
    if (userRole === ROLES.LIBRARY_OWNER) {
      ...
      req.libraryFilter = { tenant_id: req.user.tenant_id };
      return next();
    }

    // Other roles can only access their specific library/branch
    if (req.user.library_id) {
      req.libraryFilter = { library_id: req.user.library_id };
      return next();
    }
