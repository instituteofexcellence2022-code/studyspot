...
class ReviewRepository {
  ...
  async create(reviewData) {
    const { libraryId, userId, bookingId, rating, comment } = reviewData;

    const result = await query(
      `INSERT INTO reviews (library_id, user_id, booking_id, rating, comment, is_verified)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [libraryId, userId, bookingId, rating, comment, bookingId ? true : false]
    );
  ...
  async findByUserId(userId, options = {}) {
    ...
    const result = await query(
      `SELECT r.*, l.name as library_name, l.city
       FROM reviews r
       JOIN libraries l ON r.library_id = l.id
       WHERE r.user_id = $1
  ...
  async getLibraryStats(libraryId) {
    const result = await query(
      `SELECT 
       ...
       FROM reviews
       WHERE library_id = $1 AND is_flagged = false`,
      [libraryId]
    );
  ...
  async getRecentReviews(limit = 10) {
    const result = await query(
      `SELECT r.*, u.first_name, u.last_name, l.name as library_name, l.city
       FROM reviews r
       JOIN users u ON r.user_id = u.id
       JOIN libraries l ON r.library_id = l.id
