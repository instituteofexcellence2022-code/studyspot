...
class BookingRepository {
  ...
  async create(bookingData) {
    ...
    const result = await query(
      ...
      RETURNING *`,
      [userId, libraryId, seatId, seatNumber, bookingDate, startTime, endTime, 
       durationHours, amount, paymentMethod]
    );
  ...
  async findById(bookingId) {
    const result = await query(
      `SELECT b.*, l.name as library_name, l.address, l.city, l.state,
  ...
  async findByLibraryId(libraryId, options = {}) {
    const { bookingDate, status } = options;
  ...
  async updatePaymentStatus(bookingId, paymentStatus, paymentData = {}) {
    ...
    const result = await query(
      `UPDATE bookings 
       ...
       WHERE id = $5
       RETURNING *`,
      [paymentStatus, razorpayOrderId, razorpayPaymentId, stripePaymentIntentId, bookingId]
    );
  ...
  async getAvailableSeats(libraryId, bookingDate, startTime, endTime) {
    const result = await query(
      ...
      [libraryId, bookingDate, startTime, endTime]
    );
