...
class FCMService {
  ...
  async sendToUser(userId, notification, data = {}) {
    try {
      ...
      if (!Array.isArray(tokens) || tokens.length === 0) {
        logger.warn('Invalid FCM tokens for user', { userId });
        return { success: false, error: 'Invalid device tokens' };
      }
  ...
  async registerDevice(userId, token, deviceInfo = {}) {
    try {
      ...
      if (!tokens.includes(token)) {
        ...
        await query(
          'UPDATE users SET fcm_tokens = $1, updated_at = NOW() WHERE id = $2',
  ...
  async cleanupInvalidTokens(userId) {
    try {
      ...
      await query(
        'UPDATE users SET fcm_tokens = $1, updated_at = NOW() WHERE id = $2',
        [JSON.stringify(validTokens), userId]
      );
