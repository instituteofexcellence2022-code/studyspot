      ...
      Object.keys(eventData).forEach(dataKey => {
        if (typeof eventData[dataKey] === 'number') {
          if (!metric.data[dataKey]) {
            metric.data[dataKey] = { sum: 0, count: 0, avg: 0 };
          }
  ...
  analyzeWeekdayPattern(data) {
    ...
    data.forEach(d => {
      ...
      if (!weekdayBookings[dayName]) {
        weekdayBookings[dayName] = { total: 0, count: 0 };
      }
  ...
  async detectAnomalies(dateFrom = null, dateTo = null) {
    try {
      ...
      const stdDev = Math.sqrt(
        data.reduce((sum, d) => sum + Math.pow(d.totalBookings - avgBookings, 2), 0) / data.length
      );
      ...
      return {
        ...
        data: {
          ...
          insights: anomalies.length > 0 
            ...
            : 'No significant anomalies detected - data is stable'
        }
      };
    } catch (error) {
