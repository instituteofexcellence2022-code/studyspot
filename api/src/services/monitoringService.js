...
class MonitoringService {
  constructor() {
    this.errorThresholds = {
      ...
      response_time: 2000, // 2 seconds response time threshold
  ...
  async checkErrorThresholds() {
    const recentErrors = this.metrics.errors.filter(
      error => new Date(error.timestamp) > new Date(Date.now() - 5 * 60 * 1000) // Last 5 minutes
    );
  ...
  async checkPerformanceThresholds() {
    const recentPerf = this.metrics.response_times.filter(
      perf => new Date(perf.timestamp) > new Date(Date.now() - 5 * 60 * 1000) // Last 5 minutes
    );
  ...
  async sendAlert(alertType, data) {
    ...
    if (this.alertCooldowns.has(alertKey)) {
      ...
      if (new Date() - lastSent < 30 * 60 * 1000) { // 30 minutes cooldown
  ...
  formatAlertMessage(alertType, data) {
    ...
    switch (alertType) {
      case 'error_rate_high':
        return baseMessage + 
          `High error rate detected: ${data.errorCount} errors in ${data.timeWindow}\n` +
          `Recent errors:\n${data.recentErrors.map(e => `- ${e.type}: ${e.message}`).join('\n')}`;
      
      case 'critical_errors':
        return baseMessage + 
          `${data.errors.length} critical errors detected:\n` +
          data.errors.map(e => `- ${e.type}: ${e.message}`).join('\n');
      
      case 'slow_response':
        return baseMessage + 
          `Slow response times: ${data.averageResponseTime}ms average (threshold: ${data.threshold}ms)\n` +
          `Sample count: ${data.sampleCount}`;
      
      case 'high_memory_usage':
        return baseMessage + 
          `High memory usage: ${data.memoryUsage.toFixed(2)}% (threshold: ${data.threshold}%)\n` +
          `Usage: ${this.formatBytes(data.usage)} / ${this.formatBytes(data.total)}`;
      
      case 'high_cpu_usage':
        return baseMessage + 
          `High CPU usage: ${data.cpuUsage}% (threshold: ${data.threshold}%)`;
  ...
  getDashboardData() {
    ...
    const recentPerf = this.metrics.response_times.filter(
      perf => new Date(perf.timestamp) > lastHour
    );

    const recentMemory = this.metrics.memory_usage.filter(
      mem => new Date(mem.timestamp) > last24Hours
    );

    const recentCpu = this.metrics.cpu_usage.filter(
      cpu => new Date(cpu.timestamp) > last24Hours
    );

    // Calculate metrics
    const errorRate = recentErrors.length / Math.max(recentPerf.length, 1) * 100;
    const avgResponseTime = recentPerf.length > 0 
      ? recentPerf.reduce((sum, perf) => sum + perf.duration, 0) / recentPerf.length 
      : 0;

    const avgMemoryUsage = recentMemory.length > 0
      ? recentMemory.reduce((sum, mem) => sum + mem.percentage, 0) / recentMemory.length
      : 0;

    const avgCpuUsage = recentCpu.length > 0
      ? recentCpu.reduce((sum, cpu) => sum + cpu.percentage, 0) / recentCpu.length
      : 0;
  ...
  updateThresholds(newThresholds) {
    ...
    logger.info('Monitoring thresholds updated', { thresholds: this.errorThresholds });
  }
  ...
  clearOldData() {
    ...
    this.alertCooldowns.forEach((value, key) => {
      if (new Date() - value > 24 * 60 * 60 * 1000) { // 24 hours
