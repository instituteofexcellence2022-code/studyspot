...
const GST_RATES = {
  STANDARD: 18, // 18% GST for standard services
...
async function generateInvoiceNumber(libraryId, financialYear = null) {
  try {
    ...
    const fy = financialYear || (currentMonth >= 4 
...
async function createInvoice(invoiceData) {
  ...
  try {
    ...
    const {
      ...
      library_id,
      payment_id,
      items,  // Array of { description, amount, quantity, hsn_code }
      library_gstin,
      library_state,
      customer_name,
      customer_address,
    ...
    const result = await client.query(insertQuery, [
      ...
      gstDetails.sgst,
      gstDetails.igst,
      gstDetails.total_gst,
      total,
      gstDetails.gst_rate,
      gstDetails.is_intra_state,
      library_gstin,
      library_state,
      customer_name,
      customer_address,
