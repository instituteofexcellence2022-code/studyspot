...
class OAuthService {
  ...
  async handleGoogleAuth(profile, accessToken) {
    try {
      ...
      logger.info('New user created via Google', { userId: newUser.id, email });
  ...
  async handleFacebookAuth(profile, accessToken) {
    try {
      ...
      if (userResult.rows.length > 0) {
        ...
        logger.info('User logged in via Facebook', { userId: user.id, email });
      ...
      if (userResult.rows.length > 0) {
        ...
        await query(
          'UPDATE users SET facebook_id = $1, last_login = NOW() WHERE id = $2',
          [facebookId, user.id]
        );

        logger.info('Facebook account linked to existing user', { userId: user.id, email });
      ...
      logger.info('New user created via Facebook', { userId: newUser.id, email });
  ...
  async linkFacebookAccount(userId, facebookId) {
    try {
      ...
      logger.info('Facebook account linked', { userId, facebookId });
      return { success: true };
    } catch (error) {
  ...
  async unlinkFacebookAccount(userId) {
    try {
      await query(
        'UPDATE users SET facebook_id = NULL, updated_at = NOW() WHERE id = $1',
        [userId]
      );
  ...
  async getConnectedAccounts(userId) {
    try {
      const result = await query(
        'SELECT google_id, facebook_id FROM users WHERE id = $1',
        [userId]
      );
