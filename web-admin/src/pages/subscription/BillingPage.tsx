...
import { PaymentMethod, Invoice, InvoiceStatus } from '../../types';
...
const BillingPage: React.FC = () => {
  ...
  const {
    invoices,
    invoicesLoading,
    invoicesError} = useAppSelector((state: any) => state.subscription);
  ...
  useEffect(() => {
    if (user?.tenantId) {
      dispatch(fetchInvoices({ tenantId: user.tenantId, limit: 10 } as any));
    }
    // dispatch(fetchPaymentMethods( as any)); // TODO: Implement when slice is ready
  }, [dispatch]);

  // Handle download invoice
  const handleDownloadInvoice = async (id: string) => {
    try {
      // await dispatch(downloadInvoicePdf(id as any)).unwrap();
  ...
  const handleDeletePaymentMethod = async (id: string) => {
    if (window.confirm('Are you sure you want to remove this payment method?')) {
      try {
        // await dispatch(deletePaymentMethod(id as any)).unwrap();
  ...
  const handleSetDefaultPaymentMethod = async (id: string) => {
    try {
      // await dispatch(setDefaultPaymentMethod(id as any)).unwrap();
