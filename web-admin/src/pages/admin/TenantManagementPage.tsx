...
import { Container, Typography, Card, CardContent, Button, TextField, MenuItem, Box, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TablePagination, InputAdornment } from '@mui/material';
import {
  Block as BlockIcon,
...
const TenantManagementPage: React.FC = () => {
  ...
  const {
    ...
    tenantsTotalPages,
    tenantsCurrentPage,
    filters} = useAppSelector((state: any) => state.tenant);
  ...
  useEffect(() => {
    dispatch(fetchTenants(filters as any));
  }, [dispatch, filters]);

  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    dispatch(setFilters({ search: e.target.value } as any));
  };

  const handleStatusFilter = (e: any) => {
    dispatch(setFilters({ status: e.target.value as TenantStatus | 'all' } as any));
  };

  const handlePageChange = (_event: unknown, newPage: number) => {
    dispatch(setFilters({ page: newPage + 1 } as any));
  };
  ...
  const handleSuspend = async () => {
    ...
    try {
      await dispatch(suspendTenant({ id: selectedTenant.id, reason: suspendReason } as any)).unwrap();
  ...
  const handleReactivate = async (id: string) => {
    try {
      await dispatch(reactivateTenant(id as any)).unwrap();
    } catch (error) {
