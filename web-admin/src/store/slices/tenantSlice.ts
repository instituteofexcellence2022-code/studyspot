...
import { tenantService, onboardingHelpers
  } from '../../services';
...
const tenantSlice = createSlice({
  ...
  reducers: {
    ...
    saveOnboardingProgress: (
      ...
      state.onboardingProgress = {
        id: state.onboardingProgress?.id || 'temp',
        tenantId: state.onboardingProgress?.tenantId || 'temp',
        currentStep: step,
        totalSteps: 6,
        ...
        data: {
          ...
          ...data},
        completed: step >= 6,
        createdAt: state.onboardingProgress?.createdAt || new Date().toISOString(),
    ...
    clearOnboardingProgress: (state: any) => {
    ...
    optimisticUpdateTenant: (
      ...
      // Update in list
      const index = state.tenants.findIndex((t: any) => t.id === id);
      if (index !== -1) {
  ...
  extraReducers: (builder: any) => {
    ...
    builder
      .addCase(fetchCurrentTenant.pending, (state: any) => {
        state.currentTenantLoading = true;
        state.currentTenantError = null;
      })
      .addCase(fetchCurrentTenant.fulfilled, (state: any, action: any) => {
        state.currentTenantLoading = false;
        state.currentTenant = action.payload;
        state.lastFetched.currentTenant = Date.now();
      })
      .addCase(fetchCurrentTenant.rejected, (state: any, action: any) => {
        state.currentTenantLoading = false;
    ...
    // Fetch Tenants List
    builder
      .addCase(fetchTenants.pending, (state: any) => {
      ...
      .addCase(fetchTenants.rejected, (state: any, action: any) => {
        state.tenantsLoading = false;
    ...
    // Fetch Tenant By ID
    builder
      .addCase(fetchTenantById.pending, (state: any) => {
        state.selectedTenantLoading = true;
        state.selectedTenantError = null;
      })
      .addCase(fetchTenantById.fulfilled, (state: any, action: any) => {
        state.selectedTenantLoading = false;
        state.selectedTenant = action.payload;
      })
      .addCase(fetchTenantById.rejected, (state: any, action: any) => {
        state.selectedTenantLoading = false;
    ...
    // Create Tenant
    builder
      .addCase(createTenant.pending, (state: any) => {
        state.onboardingLoading = true;
        state.onboardingError = null;
      })
      .addCase(createTenant.fulfilled, (state: any, action: any) => {
        state.onboardingLoading = false;
        state.currentTenant = action.payload;
        state.onboardingProgress = null;
      })
      .addCase(createTenant.rejected, (state: any, action: any) => {
        state.onboardingLoading = false;
    ...
    // Update Tenant
    builder
      .addCase(updateTenant.fulfilled, (state: any, action: any) => {
        ...
        const index = state.tenants.findIndex((t: any) => t.id === action.payload.id);
        if (index !== -1) {
    ...
    // Update Branding
    builder.addCase(updateBranding.fulfilled, (state: any, action: any) => {
      if (state.currentTenant?.id === action.payload.id) {
        state.currentTenant = action.payload;
      }
    });
    ...
    // Upload Logo
    builder.addCase(uploadLogo.fulfilled, (state: any, action: any) => {
    ...
    // Suspend Tenant
    builder.addCase(suspendTenant.fulfilled, (state: any, action: any) => {
      const index = state.tenants.findIndex((t: any) => t.id === action.payload.id);
      if (index !== -1) {
        state.tenants[index] = action.payload;
      }
    ...
    // Reactivate Tenant
    builder.addCase(reactivateTenant.fulfilled, (state: any, action: any) => {
      const index = state.tenants.findIndex((t: any) => t.id === action.payload.id);
      if (index !== -1) {
        state.tenants[index] = action.payload;
      }
    ...
    // Delete Tenant
    builder.addCase(deleteTenant.fulfilled, (state: any, action: any) => {
      state.tenants = state.tenants.filter((t: any) => t.id !== action.payload);
    ...
    // Fetch Analytics
    builder
      .addCase(fetchAnalytics.pending, (state: any) => {
        state.analyticsLoading = true;
        state.analyticsError = null;
      })
      .addCase(fetchAnalytics.fulfilled, (state: any, action: any) => {
        state.analyticsLoading = false;
        state.analytics = action.payload;
        state.lastFetched.analytics = Date.now();
      })
      .addCase(fetchAnalytics.rejected, (state: any, action: any) => {
        state.analyticsLoading = false;
...
export const {
  ...
  clearErrors,
  optimisticUpdateTenant} = tenantSlice.actions;
