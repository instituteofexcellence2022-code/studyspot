...
const initialState: UIState = {
  sidebarOpen: storage.get<boolean>('sidebar_open') ?? true,
  themeMode: storage.get<'light' | 'dark'>('theme_mode') ?? 'light',
  notifications: [],
  snackbar: {
    open: false,
    message: '',
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    toggleSidebar: (state) => {
      ...
      storage.set('sidebar_open', state.sidebarOpen);
    },
    setSidebarOpen: (state, action: PayloadAction<boolean>) => {
      state.sidebarOpen = action.payload;
      storage.set('sidebar_open', action.payload);
    },

    // Theme
    toggleTheme: (state) => {
      state.themeMode = state.themeMode === 'light' ? 'dark' : 'light';
      storage.set('theme_mode', state.themeMode);
    },
    setThemeMode: (state, action: PayloadAction<'light' | 'dark'>) => {
      state.themeMode = action.payload;
      storage.set('theme_mode', action.payload);
    },
