...
export const createAutoTopupConfig = createAsyncThunk(
  ...
  async (data: AutoTopupCreateRequest, { rejectWithValue }) => {
    try {
      ...
      return { type: data.creditType, config };
    } catch (error: any) {
...
const creditSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(refreshCreditBalance.pending, (state) => {
        state.balance.loading = true;
        state.balance.error = null;
      })
      .addCase(refreshCreditBalance.fulfilled, (state, action) => {
        state.balance.loading = false;
        state.balance.data = action.payload;
      })
      .addCase(refreshCreditBalance.rejected, (state, action) => {
        state.balance.loading = false;
        state.balance.error = action.payload as string;
      });
    ...
    // Auto Top-up
    builder
      .addCase(fetchAutoTopupConfigs.pending, (state) => {
        state.autoTopup.loading = true;
        state.autoTopup.error = null;
      })
      .addCase(fetchAutoTopupConfigs.fulfilled, (state, action) => {
        state.autoTopup.loading = false;
        state.autoTopup.sms = action.payload.sms;
        state.autoTopup.whatsapp = action.payload.whatsapp;
      })
      .addCase(fetchAutoTopupConfigs.rejected, (state, action) => {
        state.autoTopup.loading = false;
        state.autoTopup.error = action.payload as string;
      });

    builder.addCase(createAutoTopupConfig.fulfilled, (state, action) => {
    ...
    builder.addCase(updateAutoTopupConfig.fulfilled, (state, action) => {
      if (action.payload.type === 'sms') {
        state.autoTopup.sms = action.payload.config;
      } else {
        state.autoTopup.whatsapp = action.payload.config;
    ...
    builder.addCase(deleteAutoTopupConfig.fulfilled, (state, action) => {
    ...
    // Usage Stats
    builder
      .addCase(fetchCreditUsageStats.pending, (state) => {
        state.usage.loading = true;
        state.usage.error = null;
      })
      .addCase(fetchCreditUsageStats.fulfilled, (state, action) => {
        state.usage.loading = false;
        state.usage.stats = action.payload;
      })
      .addCase(fetchCreditUsageStats.rejected, (state, action) => {
        state.usage.loading = false;
    ...
    // Alerts
    builder
      .addCase(fetchCreditAlerts.pending, (state) => {
        state.alerts.loading = true;
        state.alerts.error = null;
      })
      .addCase(fetchCreditAlerts.fulfilled, (state, action) => {
        state.alerts.loading = false;
        state.alerts.items = action.payload;
        state.alerts.unreadCount = action.payload.filter((a) => !a.isRead).length;
      })
      .addCase(fetchCreditAlerts.rejected, (state, action) => {
        state.alerts.loading = false;
        state.alerts.error = action.payload as string;
      });

    builder.addCase(markAlertAsRead.fulfilled, (state, action) => {
      const alert = state.alerts.items.find((a) => a.id === action.payload);
      if (alert) {
        alert.isRead = true;
    ...
    builder.addCase(markAllAlertsAsRead.fulfilled, (state) => {
      state.alerts.items.forEach((alert) => {
        alert.isRead = true;
      });
    ...
    builder.addCase(deleteAlert.fulfilled, (state, action) => {
      state.alerts.items = state.alerts.items.filter((a) => a.id !== action.payload);
      state.alerts.unreadCount = state.alerts.items.filter((a) => !a.isRead).length;
    });
  },
});
